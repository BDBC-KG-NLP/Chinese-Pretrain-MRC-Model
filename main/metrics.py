...
def evaluate(ref_ans, pred_ans, verbose=False):
    ...
    f1_score = 100.0 * f1 / (total_count-skip_count+1)
    em_score = 100.0 * em / (total_count-skip_count+1)
...
def baidu_evaluate(ref_file, pred_file, verbose=False):
    ...
    line = f.readline()
    while line:
        ref_ans.append(json.loads(line))
        line = f.readline()
...
def compute_predictions_logits(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        if not features:
            print(features)
            all_predictions[example.qas_id] = "no features"
            all_nbest_json[example.qas_id] = []
            scores_diff_json[example.qas_id] = 1000000
            continue
        ...
        if not version_2_with_negative:
            all_predictions[example.qas_id] = nbest_json[0]["text"].replace(' ', '')
            # 去掉空格分数更高?
            # all_predictions[example.qas_id] = nbest_json[0]["text"].replace(' ', '')
        else:
            ...
            if best_non_null_entry:
                ...
                if score_diff > null_score_diff_threshold:
                    ...
                    all_predictions[example.qas_id] = best_non_null_entry.text.replace(' ', '')
            else:
                all_predictions[example.qas_id] = "no answer"
...
def compute_predictions_log_probs(
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        # just create a nonce prediction in this case to avoid failure.
        if not nbest:
            nbest.append(_NbestPrediction(text="no answer", start_log_prob=-1e6, end_log_prob=-1e6))
