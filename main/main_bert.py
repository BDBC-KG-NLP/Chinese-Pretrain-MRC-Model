        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            try:
                ...
                if "out of memory" in str(exception):
                    logger.info("WARNING: out of memory, exception: %s", str(exception))
                    logger.info("OOM: memory allocated: %s; memory reserved: %s", torch.cuda.memory_allocated(),
                                torch.cuda.memory_reserved())
                if hasattr(torch.cuda, 'empty_cache'):
                    logger.info("try to empty_cache")
                    torch.cuda.empty_cache()
                    logger.info("AFTER FREE CACHE: memory allocated: %s; memory reserved: %s", torch.cuda.memory_allocated(),
                                torch.cuda.memory_reserved())
...
def merge(args, tokenizer, prefix="test"):
    ...
    for r0,r1,r2,r3,r4 in zip(all_results_kf0, all_results_kf1, all_results_kf2, all_results_kf3, all_results_kf4):
        ...
        start_logits = np.mean([r0.start_logits, r1.start_logits, r2.start_logits, r3.start_logits, r4.start_logits], axis=0)
...
def load_and_cache_examples(args, tokenizer, set_type='train', output_examples=False):
    ...
    input_dir = args.feature_dir if args.feature_dir else r"./save_cache"
    ...
    print(cached_features_file, os.path.exists(cached_features_file), args.overwrite_cache)
    ...
    logger.info("return dataset from cached file %s", cached_features_file)
    if output_examples:
...
def main():
    ...
    if args.do_train:
        ...
        for start_point in range(start, end, span):
            logger.info("start point %s", start_point)
            args.data_start_point = start_point
            logger.info("Before Dataset: memory allocated: %s; memory reserved %s",
            ...
            logger.info("After Dataset: memory allocated: %s; memory reserved %s",
                        torch.cuda.memory_allocated(), torch.cuda.memory_reserved())
            logger.info("get dataset to train!!!!!!")
            global_step, tr_loss = train(args, train_dataset, model, tokenizer)
            logger.info("finish train function!!!!!!\n global_step = %s, average loss = %s", global_step, tr_loss)
